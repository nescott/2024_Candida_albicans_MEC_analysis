...
## Sequencing reads
All sequencing data generated by this study has been submitted to SRA 
(NCBI BioProject PRJNA1068683).
...
and duplicate marking with samtools v1.10 
(see "scripts/classify_aliqn_qc/array_bbduk_align_sort.sh").
...
Isolates with low percentage of aligned reads were investigated via
...
than originally ID'd by the clinical laboratory (MEC008 removed from dataset
and  MEC005 added).
...
with each chromosome as a separate region (see "scripts/variants/array_alb_freebayes.sh").
...
generate a matrix for clustering ("scripts/variants/concat_filter_annotate.sh"). 
Multiple correspondence analysis was performed using the FactoMiner package v2.9 
(see "scripts/clustering/"). 
...
## Combined data:  variant calling, annotation, filtering
...
"scripts/haplotype_caller_scripts/"). Variants were called for all study isolates with 
